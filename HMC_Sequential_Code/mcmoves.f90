module mcmoves
  ...
  !use lammps, only: lammps_command, lammps_scatter_atoms, lammps_gather_atoms, &
  !    lammps_extract_compute, lammps_extract_global, LMP_STYLE_GLOBAL, LMP_TYPE_SCALAR
...
contains
  ...
  subroutine MD_move(lmp,MyRank,Communicator,Temperature,TrajectoryLength,Species,Bias,CutOffDistance,Cluster,naccpt)
    ...
    integer, intent(in) :: MyRank, TrajectoryLength
    ...
    write(command_str,'(a4,i10,a16)')'run ',TrajectoryLength, ' pre yes post no'
  ...
  subroutine Volume_move(lmp,MyRank,Communicator,Temperature,Pressure,Bias,Species,CutOffDistance,Cluster,naccpt)
    ...
    real(PR) :: arg, deltaEnergy, deltaBias_m, deltaBias_s, deltaBias
    character(len=100) :: command_str
    integer :: natoms3, ierror, NumberOfMolecules
    logical :: accept
